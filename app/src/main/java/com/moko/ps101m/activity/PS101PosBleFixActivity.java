...
import com.moko.ps101m.activity.filter.PS101FilterAdvNameActivity;
import com.moko.ps101m.activity.filter.PS101FilterMacAddressActivity;
import com.moko.ps101m.activity.filter.PS101FilterRawDataSwitchActivity;
...
public class PS101PosBleFixActivity extends PS101BaseActivity implements SeekBar.OnSeekBarChangeListener {
                        ...
                        if (flag == 0x01) {
                            ...
                            switch (configKeyEnum) {
                                ...
                                case KEY_FILTER_RELATIONSHIP:
                                    ...
                                    if (savedParamsError) {
                                        ToastUtils.showToast(PS101PosBleFixActivity.this, "Opps！Save failed. Please check the input characters and try again.");
                                    } else {
    ...
    public void onFilterByMac(View view) {
        if (isWindowLocked()) return;
        Intent intent = new Intent(this, PS101FilterMacAddressActivity.class);
        startActivity(intent);
    }

    public void onFilterByName(View view) {
        if (isWindowLocked()) return;
        Intent intent = new Intent(this, PS101FilterAdvNameActivity.class);
        startActivity(intent);
    }

    public void onFilterByRawData(View view) {
        if (isWindowLocked()) return;
        Intent intent = new Intent(this, PS101FilterRawDataSwitchActivity.class);
        startActivity(intent);
    }
